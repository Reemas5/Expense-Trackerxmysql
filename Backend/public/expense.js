...
function checking_forpremium(){
    ...
    show_leaderboard.addEventListener('click',async()=>{
      try{
        const resp = await axios.get('http://3.88.47.203/premium_content',{
            headers:{
                'Authorization':token
            }
        })
...
// })
    ...
    try {
        const result = await axios.get("http://3.88.47.203/premium", {
            headers: {
                "Authorization": token,
            },
        });
                ...
                "handler": async function(response) {
                    try {
                        ...
                        const updateResponse = await axios.post(
                            "http://3.88.47.203/updatetransactionstatus", {
...
function fetchexpenses(page = 1) {
    ...
    axios
      .get(`http://3.88.47.203/getexpense?page=${page}&items=${itemsPerPage}`, {
        headers: { 'Authorization': token },
      })
      .then((response) => {
...
const displayitem = (item) => {
    ...
    del_btn.addEventListener('click', async() => {
      ...
      await axios.delete(`http://3.88.47.203/delexpense/${_id}`,{
        headers:{
            "Authorization":token
        }
      })
...
const handleformsubmit =async(event)=>{
    ...
    event.preventDefault();
          ...
          const expenses = {
            ...
            try {
                const response = await axios.post('http://3.88.47.203/postexpense',expenses,
                    {
                        headers: {
                            "Authorization": token, 
                        },
